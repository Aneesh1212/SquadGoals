...
struct EncouragementModal: View {
    ...
    var body: some View {
        ...
        VStack(spacing: 24) {
            ...
            Button(action: {
                UtilFunctions.sendNotification(users: viewModel.user.teammates + [viewModel.user], title: "Squad Goals: Midweek Encouragement", message: self.customMessage == placeholder ? "There's time left in the week, lets finish a few more goals! - \(String(viewModel.user.name))" : "\(self.customMessage) - \(String(viewModel.user.name))")
