...
struct SundayPlanning: View {
    ...
    var body: some View {
        ...
        .alert("Please note weekly targets will be cleared every SUNDAY @ 5PM. If need be, edit your targets to fit this timeline", isPresented: $showEditWarning) {
            Button("Submit") {
                self.navigateToHome = true
                let defaults = UserDefaults.standard
                let previousSunday = Calendar(identifier: .gregorian).startOfDay(for: Date.today().previous(.sunday, considerToday: true))
                defaults.set(previousSunday, forKey: "lastSetSunday")
                saveGoals()
            }
